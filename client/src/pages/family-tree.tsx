                ...
                {!selectedMember ? (
                  ...
                  <Tabs defaultValue="info">
                    <TabsList className="w-full">
                      ...
                      <TabsTrigger value="narrative" className="flex-1">Story</TabsTrigger>
                      ...
                      <ScrollArea className="h-[360px] pr-4">
                        {isDetailLoading ? (
                          ...
                          <div className="space-y-4">
                            <h4 className="text-lg font-medium">Family Story</h4>
                            <p className="text-muted-foreground">
                              {selectedMember.name}'s story is part of your family's legacy. Their experiences, challenges, 
                              and triumphs have shaped not only their life but the lives of everyone connected to them.
                            </p>
                            <div className="bg-primary/5 p-4 rounded-lg">
                              <h5 className="font-medium mb-2">Generational Timeline</h5>
                              <div className="space-y-2">
                                {selectedMember.birth_date && (
                                  <div className="flex items-start space-x-2">
                                    <div className="w-2 h-2 rounded-full bg-primary mt-2"></div>
                                    <div>
                                      <span className="text-sm font-medium">{new Date(selectedMember.birth_date).getFullYear()}</span>
                                      <p className="text-sm text-muted-foreground">Born in {selectedMember.location || 'unknown location'}</p>
                                    </div>
                                  </div>
                                )}
                                {selectedMember.role === 'Father' || selectedMember.role === 'Mother' ? (
                                  <div className="flex items-start space-x-2">
                                    <div className="w-2 h-2 rounded-full bg-primary mt-2"></div>
                                    <div>
                                      <span className="text-sm font-medium">Family Formation</span>
                                      <p className="text-sm text-muted-foreground">Started a family and became a {selectedMember.role.toLowerCase()}</p>
                                    </div>
                                  </div>
                                ) : null}
                                {selectedMember.role.includes('Step') ? (
                                  <div className="flex items-start space-x-2">
                                    <div className="w-2 h-2 rounded-full bg-primary mt-2"></div>
                                    <div>
                                      <span className="text-sm font-medium">Blended Family</span>
                                      <p className="text-sm text-muted-foreground">Joined a blended family</p>
                                    </div>
                                  </div>
                                ) : null}
                              </div>
                    ...
                    <TabsContent value="relationships" className="py-4">
                      <ScrollArea className="h-[360px] pr-4">
                        <div className="space-y-4">
                          <h4 className="text-lg font-medium">Family Connections</h4>
                          <div className="space-y-3">
                            {selectedMember.role === 'Father' || selectedMember.role === 'Mother' ? (
                              <div className="flex items-center justify-between p-2 bg-background border rounded-lg">
                                <span>Parent to children</span>
                                <span className="text-sm px-2 py-1 rounded bg-green-100 text-green-800">Strong bond</span>
                              </div>
                            ) : null}
                            {selectedMember.role === 'Son' || selectedMember.role === 'Daughter' ? (
                              <div className="flex items-center justify-between p-2 bg-background border rounded-lg">
                            ...
                            {selectedMember.role.includes('Step') ? (
                              <div className="flex items-center justify-between p-2 bg-background border rounded-lg">
                                ...
                                <span className="text-sm px-2 py-1 rounded bg-yellow-100 text-yellow-800">Building bond</span>
                              </div>
                            ) : null}
                    ...
                    <TabsContent value="insights" className="py-4">
                      <ScrollArea className="h-[360px] pr-4">
                        <div className="space-y-4">
                          <h4 className="text-lg font-medium">Relationship Insights</h4>
                          <div className="space-y-3">
                            <div className="bg-primary/5 p-4 rounded-lg">
                              <h5 className="font-medium">Connection Strength</h5>
                              <div className="mt-2 h-2 bg-primary/20 rounded-full overflow-hidden">
                                {selectedMember.relationship === 'biological' ? (
                                  <div className="h-full bg-primary w-[85%]"></div>
                                ) : selectedMember.relationship === 'adoptive' ? (
                            ...
                            <div className="bg-primary/5 p-4 rounded-lg">
                              <h5 className="font-medium">Interaction Frequency</h5>
                            ...
                            <div className="bg-primary/5 p-4 rounded-lg">
                              ...
                              {selectedMember.interests && selectedMember.interests.length > 0 ? (
                                ...
                                <p className="text-sm text-muted-foreground mt-2">No common interests recorded</p>
                              )}
                            </div>
                          </div>
                        </div>
